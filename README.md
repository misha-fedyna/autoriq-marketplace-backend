...
curl -X POST http://localhost:8080/api/users/auth/jwt/create/ \
...
curl -X POST http://localhost:8080/api/cars/advertisements/ \
  -H "Authorization: Bearer <your_jwt_token>" \
...
Адмін-інтерфейс Django доступний за адресою http://localhost:8080/admin/ після входу з обліковими даними суперкористувача.
...
---
### 💡 Порада
Для зручного тестування API рекомендуємо використовувати Postman або інтерактивну документацію, доступну за адресою http://localhost:8080/api/schema/swagger-ui/